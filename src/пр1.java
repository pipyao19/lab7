...
public class пр1 {
        public static void main(String[] args) {
            try {
                ...
                if (!folder.exists())
                    folder.mkdir();              //создание папки "My" на диске, если не существует
                File f1 = new File("C:\\Users\\user\\Documents\\My\\num1Mart.txt");
                f1.createNewFile();              //создание файла в папке
...
// Прочитать числа из файла и вывести на экран
                ...
                for (int k = 0; k < count; k++) { // k – номер просмотра
                    for (int i = 0; i < count - k -1; i++) { // i – номер числа
                        ...
                        int number2 = rf.readInt();                 //чтение i-го и (1+i)-го чисел в переменные
                        if (number1 > number2) { // условие для сортировки по возрастанию
                            rf.seek(i * 4);
                            rf.writeInt(number2);             //возврат указателя на i-ое число и
                            rf.writeInt(number1);             //перестановка(запись чисел в обратном порядке)
                        } } }
